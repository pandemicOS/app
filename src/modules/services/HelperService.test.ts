...
describe('HelperService', () => {
  ...
  describe('checkForUpdates', () => {
    it('should update the app when an update is available', async () => {
      ...
      expect(expo.Updates.checkForUpdateAsync).toHaveBeenCalled();
      expect(expo.Updates.fetchUpdateAsync).toHaveBeenCalled();
      expect(expo.Updates.reloadFromCache).toHaveBeenCalled();
    });
